...
def build_cli_parser():
    parser = optparse.OptionParser(usage="%prog [options]", description="Enumerate unresolved hosts by severity")
...
def main(argv):
    ...
    unresolved_hosts = cb.detect_unresolvedhostsbyseverity(opts.count, opts.sort)

    host_num = 1
    for host in unresolved_hosts:
        print ""
        print "Host Number %s" % host_num
        print "-"*50
        host_num = host_num + 1
